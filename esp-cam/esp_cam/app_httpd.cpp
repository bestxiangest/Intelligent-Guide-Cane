...
static esp_err_t health_handler(httpd_req_t *req)
{
    ...
    snprintf(json_response, sizeof(json_response),
             "{\"ok\":true,\"sensor\":%s,\"web_port\":80,\"stream_port\":81,\"capture\":\"/capture\",\"stream\":\"/stream\",\"framesize\":%u,\"quality\":%u,\"brightness\":%d,\"contrast\":%d,\"saturation\":%d}",
             s ? "true" : "false",
...
int startCameraServer()
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "Web server ready on port 80: /health /capture /stream /status /");
    }
    else
    {
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "Stream server ready on port 81: /health /stream");
    }
    else
    {
